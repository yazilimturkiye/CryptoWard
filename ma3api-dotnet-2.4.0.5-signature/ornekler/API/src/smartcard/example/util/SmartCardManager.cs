...
//Aşağıda bulunan 2 kütüphane, .NET FrameWork uygulamaları içindir.
//.NET ve .NET Core uygulamaları için, bu satırları siliniz.
using System.Windows.Forms;
using tr.gov.tubitak.uekae.esya.api.smartcard.gui;
using tr.gov.tubitak.uekae.esya.api.smartcard.example;
    ...
    public class SmartCardManager
    {
        ...
        private SmartCardManager()
        {
            try
            {
                ...
                {
                    //Aşağıda bulunan satır, .NET FrameWork uygulamaları içindir.
                    //.NET ve .NET Core uygulamaları için, bu satırı siliniz, altındaki comment'li satırdaki comment'i kaldırınız. Aşağıdaki uyarıyı dikkate alınız.
                    index = askOptionFromGUI(null, terminals, "Okuyucu Listesi");
                    //Unit test sırasında console'dan giriş yapılamadığı için test fonksiyonunu console uygulamasından çağırmanız gerekiyor.
                    //index = askOptionFromConsole(terminals, "Okuyucu Listesi");
                    terminal = terminals[index];
                }
        ...
        public BaseSigner getSigner(string aCardPIN, ECertificate aCert)
        {
            ...
            {
                bsc.login(aCardPIN);
                SignatureAlg signatureAlg = SignatureAlg.getConvenientSignatureAlg(aCert);
        ...
        private ECertificate selectCertificate(bool checkIsQualified, bool checkBeingNonQualified, List<byte[]> aCerts)
        {
            ...
            else
            {
                ...
                //Aşağıda bulunan satır, .NET FrameWork uygulamaları içindir.
                //.NET ve .NET Core uygulamaları için, bu satırı siliniz, altındaki comment'li satırdaki comment'i kaldırınız. Aşağıdaki uyarıyı dikkate alınız.
                int result = askOptionFromGUI(null, optionList, "Sertifika Listesi");
                //Unit test sırasında console'dan giriş yapılamadığı için test fonksiyonunu console uygulamasından çağırmanız gerekiyor.
                //int result = askOptionFromConsole(optionList, "Sertifika Listesi");
        ...
        public static int askOptionFromConsole(string[] aSecenekList, string aDescription) {
            SelectorConsole selector = new SelectorConsole();
            return selector.Select(aDescription, aSecenekList);
        
        }

        //Aşağıda bulunan fonksiyon, .NET FrameWork uygulamaları içindir.
        //.NET ve .NET Core uygulamaları için, bu fonksiyonu siliniz.
        public static int askOptionFromGUI(Control aParent, string[] aSecenekList, string aDescription)
        {
