    ...
    public class BESSign : CadesSampleBase
    {
        ...
        */
        ...
        public void testSimpleSign()
        {
            ...
            FileUtil.writeBytes(testDataDirectory.FullName + @"\BES-1.p7s", signedDocument);
        ...
        public void testSigningTimeAttrSign()
        {
            ...
            //write the contentinfo to file
            FileUtil.writeBytes(testDataDirectory.FullName + @"\BES-2.p7s", signedDocument);
        ...
        public void testRSAPSS()
        {
            ...
            //write the contentinfo to file
            FileUtil.writeBytes(testDataDirectory.FullName + @"\BES-3.p7s", signedDocument);
       ...
       [Test]
        public void testCreateSignatureInTwoSteps()
        {
            ...
            //Since the specified attributes are mandatory for bes,null is given as parameter
            //for optional attributes
            byte[] dtbs = baseSignedData.initAddingSigner(ESignatureType.TYPE_BES, cert, SignatureAlg.fromName(signer.getSignatureAlgorithmStr()), signer.getAlgorithmParameterSpec(), null, params_);
            byte[] bsdBytes = baseSignedData.getEncoded();
            ...
            // add signature to bsdBytes
            finishSigning(bsdBytes, signature);

            // Read file written in finishSigning method
            byte[] signedDocument = FileUtil.readBytes(getTestDataFolder() + "BES-1.p7s");
        ...
        {
            // bsdBytes to baseSignedData
            BaseSignedData baseSignedData = new BaseSignedData(bsdBytes);

            // add signature to baseSignedData
            baseSignedData.finishAddingSigner(signature);

            byte[] signedDocument = baseSignedData.getEncoded();
            FileUtil.writeBytes(getTestDataFolder() + "BES-1.p7s", signedDocument);
        }
    }
}